...
gamma = Expression('x[0] >= 0.4 && x[0] <= 0.6 && x[1] >= 0.4 && x[1] <= 0.6 ? 0.03 : 0.01')
sigma = Expression('x[0] >= 0.2 && x[0] <= 0.8 && x[1] >= 0.2 && x[1] <= 0.3 ? 0.2 : 0.1')
...
while not (converged or iterations == maxiter):
    ...
    if sigma_error < 3 and not reduced:
        reduced = True
        gamma_learn_rate /= 2
        sigma_learn_rate /= 2
